...
namespace SmartOdyssey
{
    ...
    public static class SmartOdyssey_WorldInterface_CheckOpenOrCloseInspectPane_Patch
    {
        ...
        {
            if (__instance?.selector?.AnyObjectOrTileSelected == true && __instance?.selector?.SelectedTile.Tile?.Mutators?.Count > 0 && WorldRendererUtility.WorldSelected && (Current.ProgramState != ProgramState.Playing || Find.MainTabsRoot.OpenTab == null))
            {
                Find.WindowStack.ImmediateWindow(32520, InfoRectProps, WindowLayer.GameUI, delegate
                {
                    ...
                    Widgets.Label(rect, "SO_FeaturesInThisTile".Translate());
